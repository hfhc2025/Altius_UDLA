...
import TablaPorSemana    from '@/components/TablaPorSemanaDiplo'

import { useTopCarreras }      from '@/hooks/useTopCarreras'
import { useTotalesFunnelsD }  from '@/hooks/useTotalesFunnelsD' // ✅ el correcto

const AnimatedFunnelDynamic = nextDynamic<{
  kpis: import('@/components/AnimatedFunneldiplomado').KPI
  tipoBase: 'Todas' | 'Lead' | 'Stock'
}>(() => import('@/components/AnimatedFunneldiplomado'), { ssr: false })
...
export default function DiplomadoPage() {
  const [tab, setTab] = useState<TabId>('diplomado')
  ...
  const {
    ...
    error: errFunnel
  } = useTotalesFunnelsD(tipoBase) // ✅ hook correcto para funnel de diplomado
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 via-slate-100/50 to-white">
      ...
      <div className="flex flex-1 overflow-hidden">
        <Sidebar activeTab="diplomado" onTabChange={(t: TabId) => setTab(t)} />
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                {carreras.length > 0 && (
                  <CarrerasTopComboChart data={carreras} onFiltersChange={handleFiltersChange} />
                )}
              </div>
            </div>
          </section>
