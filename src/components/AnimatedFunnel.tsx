...
interface Props {
  kpis: KPI[] // ahora espera arreglo porque viene toda la vista SQL
  tipoBase: 'Todas' | 'Lead' | 'Stock'
}

/* ── Átomo visual de cada etapa ───────────────────────────── */
...
export default function AnimatedFunnel({ kpis, tipoBase }: Props) {
  const fila = useMemo(() => {
    if (!Array.isArray(kpis)) return null
    if (tipoBase === 'Todas') return kpis.find(k => k.tipo_base === 'Total')
    return kpis.find(k => k.tipo_base === tipoBase)
  }, [kpis, tipoBase])
  ...
  const stages: Stage[] = [
    { key: 'base', label: 'Base Total', value: v.total_base,
    ...
    { key: 'reco', label: 'Recorrido', value: v.recorrido,
      percentage: pct(v.recorrido, v.total_base),
      conversionRate: conv(v.recorrido, v.total_base),
    ...
    { key: 'cont', label: 'Contactados', value: v.contactados,
      percentage: pct(v.contactados, v.total_base),
    ...
    { key: 'cita', label: 'Citas', value: v.citas,
      percentage: pct(v.citas, v.total_base),
    ...
    { key: 'aflu', label: 'Afluencias', value: v.afluencias,
      percentage: pct(v.afluencias, v.total_base),
    ...
    { key: 'matr', label: 'Matrículas', value: v.matriculas,
      percentage: pct(v.matriculas, v.total_base),
